...
export default function NewService() {
  ...
  useEffect(() => {
    function handleServiceWorkerMessage(event: MessageEvent) {
      ...
      // Quando receber notificação de operação offline iniciada
      if (event.data.type === 'offline-operation-started') {
        console.log("Operação offline iniciada, ID:", event.data.tempId);
        
        // Verificar se estamos em processo de salvamento
        const isMutationPending = document.querySelector('button[type="submit"]:disabled');
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <LocationSelector
                      ...
                      onChange={(location) => {
                        form.setValue('location_type', location.locationType);
