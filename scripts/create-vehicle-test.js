...
async function testCreateVehicle() {
  try {
    ...
    if (vehiclesTableCheck.length === 0) {
    ...
    console.log('Tipo de insertId:', typeof result.insertId);
    
    // Buscar o veículo inserido
    const [vehicles] = await connection.query('SELECT * FROM vehicles WHERE id = ?', [result.insertId]);
    console.log('Veículo inserido:', vehicles[0]);
