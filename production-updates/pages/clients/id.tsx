...
export default function ClientDetail({ id }: ClientDetailProps) {
  ...
  const { 
    ...
    queryFn: async () => {
      try {
        console.log(`Buscando veículos do cliente ${clientId} - ${new Date().toISOString()}`);
                ...
                {vehicles.length > 0 && (
                  <div className="flex justify-end mt-4">
                    <Button
                      variant="outline"
                      onClick={() => {
                        setLocation(`/clients/${client.id}/vehicle/new`);
                      }}
                    >
                ...
                {isLoadingServices ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground">
                    ...
                    <Button 
                      variant="outline" 
                      className="mt-4"
                      onClick={() => setLocation(`/services/new?client=${client.id}`)}
                    >
                      Cadastrar Serviço
                    </Button>
                  </div>
